...
export async function router(req, res){
    try {
        ...
        const possibleEndpoints = ENDPOINTS.filter(e => e.pattern.test(url.pathname));
        ...
        for (const endpoint of possibleEndpoints) {
            if (req.method === endpoint.method) {
                ...
                req.params = endpoint.params.reduce((acc, param) => ({...acc, [param.name]: segments[param.idx]}), {});
            ...
            import(path.join(dir, fullPath)).then(route => {
                for (const handler of Object.values(route)) {
                    if (ALLOWED_METHODS.includes(handler.name)) {
                        ...
                        ENDPOINTS.push(new Endpoint(handler.name, handler, pattern, params));
                    }
                }
            })
        }
